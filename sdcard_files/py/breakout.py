# breakout.py -- adapte pour l'AKA (ecran 320x240, module aka natif) depuis
...
#
# BUG EVITE : le module "math" (sqrt) n'est pas disponible sur cette config
# MicroPython minimale (verifie dans mpconfig.h) -- utilise l'implementation
# pure Python fournie par game8266.py a la place.
# BUG EVITE : const() n'est pas non plus garanti disponible -- repli
# trivial, sans effet si const existe deja reellement.
def const(x): return x

# all dislplay, buttons, paddle, sound logics are in game8266.py module
from game8266 import Game8266, Rect, sqrt
import aka
g=Game8266()
g.set_controls(["G/D : raquette", "L : quitter"])
paddle_width = 22
frameRate = 30
...
class Ball(object):
    ...
    def set_position(self, paddle_x, paddle_y, paddle_x2, paddle_center):
        ...
        if self.frozen:
            ...
            if (aka.ticks_ms() - self.created) >= 2000:
...
class Score(object):
    ...
    def __init__(self, display):
        ...
        margin = 5
        self.display = display
        self.display.text('S:', margin, 0, (0, 200, 255))
        self.x = 20 + margin
        self.y = 0
        self.value = 0
        self.draw()
    ...
    def draw(self):
        ...
        self.display.text( str(self.value), self.x, self.y,(0, 200, 255))
...
while not exitGame :
    ...
    while True:
        g.display.fill(0)
        g.display.text('Breakout', 0, 0, (255, 220, 0))
    ...
    if not exitGame :
      ...
      try:
          while not gameOver :
              g.getBtn()
              # BUG TROUVE ET CORRIGE : les briques n'etaient dessinees
              # QU'UNE FOIS, avant la boucle -- jamais redessinees ensuite,
              # contrairement a tous les autres elements (balle, raquette,
              # score) qui se re-affichent eux-memes a chaque frame. Les
              # redessiner explicitement ici les rend enfin visibles en
              # continu.
              for brick in bricks:
                  brick.draw()
              if g.menu_was_open:
                  # BUG TROUVE ET CORRIGE : apres fermeture du menu
                  # systeme AKA, la raquette/balle/score/vies ne se
                  # redessinaient pas forcement (elles ne se redessinent
                  # normalement que quand leur valeur change) -- restes du
                  # menu visibles par-dessus. Redessin complet force ici.
                  g.display.fill(0)
                  for brick in bricks:
                      brick.draw()
                  paddle.draw()
