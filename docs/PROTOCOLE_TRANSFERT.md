...
**Ce fichier est une copie** de `docs/PROTOCOLE_TRANSFERT.md` du dépôt AKA-Love, qui fait foi en cas de
divergence (c'est là que le protocole a été conçu et testé en premier). Copié ici pour que ce dépôt
reste lisible seul, sans devoir aller chercher AKA-Love.

Implémentation de référence : `components/akalove/transfer.cpp` (AKA-Love), `components/micropython/transfer.cpp` (MicroPython-AKA, ce dépôt) et `tools/akatransfer.py` (PC, dépôt AKA-Love). Statut : **écrit et testé sur le simulateur** (18 contrôles automatiques rien que pour le
...
- **MicroPython AKA** : fait (ce dépôt) — `aka.receive_code()`, accessible comme première entrée du
  sélecteur de jeux (`py/main.py`, « [Recevoir un code] »). Écrit et compilé avec succès en isolation
  contre les vrais en-têtes générés par `build_micropython_embed.sh` (voir le README, section Statut) ;
  jamais essayé dans le firmware complet ni sur une console.
